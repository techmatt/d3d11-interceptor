...
int Character::animationsOverlap(const CharacterInstance &seedInstance, const CharacterInstance &otherInstance, int animationLength)
{
    int overlap = 0;
    for (int offsetB = 0; offsetB < animationLength; offsetB++)
    {
        const CharacterInstance *instanceB = findInstanceAtFrame(otherInstance.frameID.delta(offsetB));
        if (instanceB == nullptr)
            return 0;

        bool found = false;
        for (int offsetA = 0; offsetA < animationLength && !found; offsetA++)
        {
            const CharacterInstance *instanceA = findInstanceAtFrame(seedInstance.frameID.delta(offsetA));
            if (instanceA == nullptr)
                return 0;

            const float distSq = math::distSq(instanceA->reducedPoseDescriptor, instanceB->reducedPoseDescriptor);
            if (distSq <= learningParams().poseDistSqThreshold)
                found = true;
        }
...
int Character::countSimilarAnimations(const CharacterInstance &seedInstance, int animationLength)
{
    auto candidateInstances = findPoseChainsRadius(seedInstance, (float)learningParams().animationDistSqBase);
}
