...
public:
	...
	{
		if(verbose) std::cout << (std::string("k-means clustering, ") + std::to_string(elements.size()) + std::string(" points, ") + std::to_string(clusterCount) + std::string(" clusters"));
		...
		while(!converged)
		{
			...
			if(verbose) std::cout << (std::string("pass ") + std::to_string(pass) + std::string(", ") + std::string("delta=") + std::to_string(delta));
		}
	}
