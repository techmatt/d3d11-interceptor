...
{
	std::cout << (std::string("running ") + std::to_string(tasks.tasksLeft()) + " tasks");
	...
	while(!allThreadsCompleted)
	{
		...
		if(consoleDelay == 0)
		{
			if(useConsole) std::cout << (std::string("tasks left: ") + std::to_string(tasks.tasksLeft() + activeThreadCount));
			consoleDelay = 40;
		}
		else
		{
	...
	if(useConsole) std::cout << ("all tasks completed");
}
