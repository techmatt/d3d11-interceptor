...
{
    if(storage.scoresLength < num_corners) return NULL;
    int* scores = storage.scores;
    int n;
...
vec2i* fast12_detect(const byte* im, int xsize, int ysize, int stride, int b, int* ret_num_corners)
{
    ...
    ret_corners = (vec2i*)malloc(sizeof(vec2i)*rsize);
    make_offsets(pixel, stride);
    ...
    for(y=3; y < ysize - 3; y++)
        for(x=3; x < xsize - 3; x++)
        {
            ...
            if(p[pixel[0]] > cb)
                ...
                {
                    rsize*=2;
                    ret_corners = (vec2i*)realloc(ret_corners, sizeof(vec2i)*rsize);
                }
...
vec2i* fast12_detect(const byte* im, int xsize, int ysize, int stride, int b, int* ret_num_corners, FastStorage &storage)
{
    int num_corners=0;
    vec2i* ret_corners;
    int rsize=storage.cornersDetectLength;
    int pixel[16];
    int x, y;

    ret_corners = storage.cornersDetect;
    make_offsets(pixel, stride);
